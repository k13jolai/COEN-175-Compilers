...
static void topLevelDeclaration()
{
	...
	if (lookahead == '[') {
	    match('[');
	    topSymbols.push_back(declareVariable(name, Type(typespec, indirection, number())));
	    match(']');
		...
		if(topSymbols.size()!=0){
			...
			cout<<"\t.comm\t"<< topSymbols[i]->name()<< ", "<< topSymbols[i]->type().size()<< ", ";
			if(topSymbols[i]->type().specifier()=="char" && topSymbols[i]->type().indirection()==0)
				cout<<"1"<<endl;
			else
				cout<<"4"<<endl;
			}
	   ...
	   topSymbols.push_back(declareVariable(name, Type(typespec, indirection)));;
